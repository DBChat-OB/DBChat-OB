        ...
        if (condition.left_is_attr == 1 && condition.right_is_attr == 1 &&
            ...
            for (int j = 0; j < selects.relation_num; j++) {
                if (strcmp(condition.left_attr.relation_name, selects.relations[j]) == 0) {
                    ...
                    int id = tupleSchemas[selects.relation_num-1-j].index_of_field(condition.left_attr.relation_name,
                                                                                   condition.left_attr.attribute_name);
                    if (id == -1) {
                        Table *table = DefaultHandler::get_default().find_table(db, condition.left_attr.relation_name);
                        RC rc = schema_add_field(table, condition.left_attr.attribute_name, tupleSchemas[selects.relation_num-1-j]);
                        if (rc != RC::SUCCESS) {
                ...
                if (strcmp(condition.right_attr.relation_name, selects.relations[j]) == 0) {
                    ...
                    int id = tupleSchemas[selects.relation_num-1-j].index_of_field(condition.right_attr.relation_name,
                    ...
                    if (id == -1) {
                        ...
                        RC rc = schema_add_field(table, condition.right_attr.attribute_name, tupleSchemas[selects.relation_num-1-j]);
                        if (rc != RC::SUCCESS) {
                            return RC::SCHEMA_FIELD_MISSING;
                        }
                    }
...
RC ExecuteStage::do_select(const char *db, Query *sql, SessionEvent *session_event) {
    ...
    for (int i = selects.relation_num-1; i >= 0; i--) {
        ...
        rc = create_selection_executor(trx, selects, db, table_name, *select_node, schemas[selects.relation_num-1-i]);
        if (rc != RC::SUCCESS) {
            delete select_node;
