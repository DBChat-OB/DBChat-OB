...
RC DefaultHandler::create_index(Trx *trx, const char *dbname, const char *relation_name, const char *index_name, const char *attribute_name, bool unique_attr) {
  Table *table = find_table(dbname, relation_name);
  if (nullptr == table) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
  return table->create_index(trx, index_name, attribute_name, unique_attr);
}

RC DefaultHandler::create_multi_index(Trx *trx, const char *dbname, const char *relation_name, const char *index_name,
                                      const char **attribute_names, const size_t attribute_count) {
    Table *table = find_table(dbname, relation_name);
    if (nullptr == table) {
        return RC::SCHEMA_TABLE_NOT_EXIST;
    }
    return table->create_multi_index(trx,index_name,attribute_names,attribute_count);
}
...
RC DefaultHandler::update_record(Trx *trx, const char *dbname, const char *relation_name, const char *attribute_name, const Value *value,
  ...
  if (nullptr == table) {
    ...
    RC err;
  if ((err = table->update_record(trx, attribute_name, value, condition_num, conditions, updated_count)) != RC::SUCCESS) {
      LOG_ERROR("Update failed. %d:%s", err, strrc(err));
  }
  return err;
}
